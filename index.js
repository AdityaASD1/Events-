const elements = {
    events: document.querySelector('.bgcolor'),
    all: document.querySelectorAll('.all'),
    icon: document.querySelector('.Icon'),
    decode: document.querySelector('.Dcode'),
    drift_racing: document.querySelector('.Drift_racing'),
    cryto_rush: document.querySelector('.Cryto_rush'),
    game_jam: document.querySelector('.Game_jam'),
    xxx: document.querySelector('.XXX'),
    yyy: document.querySelector('.YYY'),
    zzz: document.querySelector('.ZZZ'),
    electronica: document.querySelector('.Electronica'),
    brain_wiz: document.querySelector('.Brain_wiz'),
    icon_title: document.querySelector('.title1'),
    decode_title: document.querySelector('.title2'),
    drift_racing_title: document.querySelector('.title3'),
    cryto_rush_title: document.querySelector('.title4'),
    game_jam_title: document.querySelector('.title5'),
    xxx_title: document.querySelector('.title6'),
    yyy_title: document.querySelector('.title7'),
    zzz_title: document.querySelector('.title8'),
    electronica_title: document.querySelector('.title9'),
    brain_wiz_title: document.querySelector('.title10')
};

let all_event = {};
for (let i = 1; i <= 10; i++) {
    all_event[`event${i}_var`] = [0, false];
}

elements.all.forEach(element => {
    element.addEventListener('click', () => {
        for (let i = 1; i <= 10; i++) {
            const varKey = `event${i}_var`;
            const titleKey = `title${i}`;
            const title = elements[titleKey];
            const titleText = (all_event[varKey][0] === 1) ? 'Events' : (i === 1) ? 'ICON' : (i === 2) ? 'Decode' : (i === 3) ? 'Drift Racing' : (i === 4) ? 'Crypto Rush' : (i === 5) ? 'Game Jam' : (i === 6) ? 'XXX' : (i === 7) ? 'YYY' : (i === 8) ? 'ZZZ' : (i === 9) ? 'Electronica' : 'Brain Wiz';
            title.innerHTML = `<h1><b>${titleText}</b></h1>`;
        }
    });
});

for (let i = 1; i <= 10; i++) {
    const element = elements[`title${i}`];
    element.addEventListener('click', () => {
        for (let j = 1; j <= 10; j++) {
            const varKey = `event${j}_var`;
            all_event[varKey][0] = 0;
            elements[`title${j}`].style.left = '-60vw';
        }
        const targetVar = `event${i}_var`;
        const targetElement = elements[Object.keys(elements).find(key => key.includes(elements[element].className.split(' ')[0]))];
        const leftElements = Object.values(elements).filter(value => value !== targetElement && value !== elements.events);
        leftElements.forEach(leftElement => leftElement.style.left = '-60vw');
        all_event[targetVar][0] = 1;
        all_event[targetVar][1] = !all_event[targetVar][1];
        if (all_event[targetVar][1]) {
            targetElement.style.left = '0vw';
            leftElements.forEach(leftElement => all_event[Object.keys(all_event).find(key => key.includes(leftElement.className.split(' ')[0]))][1] = false);
        } else {
            elements.events.style.left = '0vw';
            elements.icon.style.left = '0vw';
            all_event.event1_var[1] = false;
        }
    });
}
